...
var ZYFILE = {
    ...
    funUploadFiles: function () {
        ...
        //http://localhost:7301/ProjectWeb/sharedService/Ashx_Question.ashx?method=updateProcessInfo&id=1&imgs=test&content=test&qStatus=1
        console.log("Ashx_Question.ashx?method=updateProcessInfo&token="+token+"&id="+Data.OID+"&imgs="+images+"&content="+"&qStatus="+qstatus1);
        $.ajax({
            type: "get",
            url: svc_own + "Ashx_Question.ashx?method=updateProcessInfo&token="+token+"&id="+Data.OID+"&imgs="+images+"&content="+$("#question").val()+"&qStatus="+qstatus1,
            dataType: 'jsonp',
            jsonp: 'callback',
            data: null,
            success: function (result) {
                console.log(result);
                if (result.isOK) {
                    $("#myPopup1").html(" <h2 style=\"text-align:center;\">提示</h2><p>你已经成功提交</p>")
                    $("#myPopup1").popup('open');
