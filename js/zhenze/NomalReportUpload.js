...
var ZYFILE = {
    ...
    // 上传多个文件，2016年3月27日 Van修改
    funUploadFiles: function () {
        var self = this;  // 在each中this指向没个v  所以先将this保留
        // 遍历所有文件  ，在调用单个文件上传的方法，弃用
        ...
        // /*--------------------传图同时也传参----------------------------*/
        // self.url = "http://58.241.38.107:8090/NJXService/Data/PhotoTakeUpload.ashx?clientID=1&name=" + $("#name").val() + "&photoContent=" + $("#photoContent").val() + "&IDCard=" + $("#IDCard").val() + "&phone=" + $("#phone").val(),
        //     /*--------------------传图同时也传参----------------------------*/
        ...
        console.log("Ashx_Question.ashx?method=reportQuestion&token="+token+"&riverCode="+riverId+"&riverName="+riverName+"&reportUserName="+userName+"&qType=1&lon="+lon+"&lat="+lat+"&address=test&rType="+selectedtype+"&rTypeName="+selectedtype+"&emergency="+emergency+"&content="+ $("#question").val()+"&imgs="+images+"&videos=test&industryImg1=test&industryImg2=test&industryImg3=test&industryImg4=test&floater=test&ilegal=test&speed=test&guardrail=test&sewage=test&riverStatus=test&maintain=test");
        $.ajax({
            type: "get",
        ...
        // images="/9d2e0454-ba56-4daf-8207-79fd3408fad7.png";
        // console.log("Ashx_Question.ashx?method=reportQuestion&token="+token+"&riverCode="+getParam("OID")+"&riverName="+getParam("riverName")+"&reportUserName=公众&qType=1&lon="+lon+"&lat="+lat+"&address=test&rType=test&emergency=test&content="+ $("#question").val()+"&imgs="+images+"&videos=test&industryImg1=test&industryImg2=test&industryImg3=test&industryImg4=test&floater=test&ilegal=test&speed=test&guardrail=test&sewage=test&riverStatus=test&maintain=test");
        // $.ajax({
        //     type: "get",
        //     url: svc_own + "Ashx_Question.ashx?method=reportQuestion&token="+token+"&riverCode="+riverCode+"&riverName="+getParam("riverName")+"&reportUserName=公众&qType=1&lon="+lon+"&lat="+lat+"&address=test&rType=异常上报&emergency=test&content="+ $("#question").val()+"&imgs="+images+"&videos=test&industryImg1=test&industryImg2=test&industryImg3=test&industryImg4=test&floater=test&ilegal=test&speed=test&guardrail=test&sewage=test&riverStatus=test&maintain=test",
        //     dataType: 'jsonp',
        //     jsonp: 'callback',
        //     data: null,
        //     success: function (result) {
    ...
    funUploadFile: function (file) {
        ...
        // /*--------------------传图同时也传参----------------------------*/
        // self.url = "http://58.241.38.107:8090/NJXService/Data/ActivityUpload.ashx?clientID=1&theme=" + $("#theme").val() + "&activityContent=" + $("#photoContent").val() + "&activityTime=" + $("#time").val() + "&phone=" + $("#phone").val()+ "&address=" + $("#adress").val(),
        //
        //     //self.url = "/WEI/Data/AddRepair.ashx?location=" + $("#xiaoqu").val() + "&userID=" + "1" + "&repairContent=" + $("#repair_content").val() + "&repairID=" + randomID,
        //     /*--------------------传图同时也传参----------------------------*/
        //     xhr.open("POST", self.url, true);
        // xhr.send(formdata);
        console.log(riverId);
        if (riverId=="")
        {
            $("#myPopup1").html(" <a href=\"#\" data-rel=\"back\" class=\"ui-btn ui-corner-all ui-shadow ui-btn ui-icon-delete ui-btn-icon-notext ui-btn-right\">关闭</a><h2 style=\"text-align:center;\"><p>"+"请选择河流"+ "</p>")
            $("#myPopup1").popup('open');
        ...
        else
        {
            ...
            var  userName=$.cookie('zzwxUserName');
            // images="/9d2e0454-ba56-4daf-8207-79fd3408fad7.png";
            console.log("Ashx_Question.ashx?method=reportQuestion&token="+token+"&riverCode="+riverId+"&riverName="+riverName+"&reportUserName="+userName+"&qType=1&lon="+lon+"&lat="+lat+"&address="+riverName+"&rType="+selectedtype+"&emergency="+emergency+"&content="+ $("#question").val()+"&imgs="+images+"&videos=test&industryImg1=test&industryImg2=test&industryImg3=test&industryImg4=test&floater=test&ilegal=test&speed=test&guardrail=test&sewage=test&riverStatus=test&maintain=test");
            $.ajax({
                type: "get",
                url: svc_own + "Ashx_Question.ashx?method=reportQuestion&token="+token+"&riverCode="+riverId+"&riverName="+riverName+"&reportUserName="+userName+"&qType=1&lon="+lon+"&lat="+lat+"&address="+riverName+"&rType="+selectedtype+"&emergency="+emergency+"&content="+ $("#question").val()+"&imgs="+images+"&videos=test&industryImg1=test&industryImg2=test&industryImg3=test&industryImg4=test&floater=test&ilegal=test&speed=test&guardrail=test&sewage=test&riverStatus=test&maintain=test",
                dataType: 'jsonp',
                jsonp: 'callback',
                data: null,
                success: function (result) {
    ...
    init: function () {  // 初始化方法，在此给选择、上传按钮绑定事件
        ...
        if (self.uploadInput) {
            ...
            this.uploadInput.addEventListener("click", function (e) {
                ...
                console.log(self.uploadFile.length);
                if ($("#photoContent").val() == '') {
                    $("#myPopup1").html(" <a href=\"#\" data-rel=\"back\" class=\"ui-btn ui-corner-all ui-shadow ui-btn ui-icon-delete ui-btn-icon-notext ui-btn-right\">关闭</a><h2 style=\"text-align:center;\">注意！</h2><p>你忘了填写内容！</p>")
                    $("#myPopup1").popup('open');
                } else if (self.uploadFile.length > 4) {
                    $("#myPopup1").html(" <a href=\"#\" data-rel=\"back\" class=\"ui-btn ui-corner-all ui-shadow ui-btn ui-icon-delete ui-btn-icon-notext ui-btn-right\">关闭</a><h2 style=\"text-align:center;\">注意！</h2><p>最多上传4张图片，点击图片可删除或编辑。请重试！</p>")
                    $("#myPopup1").popup('open');
                } else {
...
(function ($, undefined) {
    ...
    $.fn.zyUpload = function (options, param) {
        ...
        return this.each(function () {
            ...
            console.log(defaults, para.width, para.height);
            this.init = function () {
            ...
            this.createHtml = function () {
                ...
                if (para.dragDrop) {
                    ...
                    html += '						<input id="fileImage" type="file" size="3000"style="width:500; height:200" name="fileselect[]" ' + multiple + '>';
                    html += '					</div>';
                    html += '				</div>';
                    ...
                    html += '				<div id="status_info" class="info">选中0张文件，共0B。</div>';
                    html += '				<div class="btns">';
                    html += '					<input id="fileImage" type="file" size="3000"style="max-width:2px;" name="fileselect[]" ' + multiple + '>';
                ...
                console.log(para.width, para.height);
