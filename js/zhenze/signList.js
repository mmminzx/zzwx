...
$(function () {
    ...
    window.addEventListener("popstate", function(e) {
        if(bool)
        {
            location.href='MyRecord.html';  //在这里指定其返回的地址
        }
        pushHistory();

    }, false);

    var value = localStorage["selectedSign"];
            ...
            success: function (result) {
                ...
                if (result.isOK) {
                    ...
                    for (var i=0;i<arr.length;i++)
                    {
                        ...
                        if (arr[i]['qType']=="2")
                        {
                            dic={"type":"异常上报","loc":[arr[i].lat,arr[i].lon]}
                        }
                        else if (arr[i]['qType']=="1")
                        {
                            dic={"type":"亮点推送","loc":[arr[i].lat,arr[i].lon]}
                        }
                        ...
                        $("#thelist").append('<li onclick="liclick11(\''+i+'\')" style="list-style: none;margin: 0 0 20px 10px;border-bottom:1px solid #616161;padding-left: 0px;" >'
                            ...
                            +'<img style="width: 50px;margin-top: 60px" src="../images/箭头.png">'
                            +'</div.>'
                            +'</li>'
                        );
                    //
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            for (var i=0;i<arr.length;i++)
            {
                var dic={"type":"工作照片","loc":[arr[i].lat,arr[i].lon]}
                toals.push(dic)
                workphotos.push(arr[i])
                var reportType="";
                ...
                $("#thelist").append('<li onclick="liclick22(\''+i+'\')" style="list-style: none;margin: 0 0 20px 10px;border-bottom:1px solid #616161;padding-left: 0px;" >'
                    +'<div style="width: 100%;height: 170px;">'
                    +'<div style="width: 85%;height:100% ;float: left;">'
                    +'<p style="color: #333333;font-size: 18px;margin-left: 0px">'+"上报类型："+reportType+'</p>'
                    +'<p style="color: #333333;font-size: 18px;margin-left: 0px">'+"河道名称："+arr[i]['riverName']+'</p>'
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            for (var i=0;i<arr.length;i++)
            {
                var dic={"type":"资料上报","loc":[arr[i].lat,arr[i].lon]}
                toals.push(dic)
                ziliao.push(arr[i])
                var reportType="";
                ...
                $("#thelist").append('<li onclick="liclick33(\''+i+'\')" style="list-style: none;margin: 0 0 20px 10px;border-bottom:1px solid #616161;padding-left: 0px;" >'
                    +'<div style="width: 100%;height: 170px;">'
                    +'<div style="width: 85%;height:100% ;float: left;">'
                    +'<p style="color: #333333;font-size: 18px;margin-left: 0px">'+"上报类型："+reportType+'</p>'
                    +'<p style="color: #333333;font-size: 18px;margin-left: 0px">'+"河道名称："+arr[i]['riverName']+'</p>'
                    // +'<p style="color: #333333;font-size: 18px;margin-left: 0px">'+"单位名称："+arr[i]['enterpriseName']+'</p>'
                    +'<p style="color: #333333;font-size: 18px;margin-left: 0px">'+"上报时间："+arr[i]['reportDate']+'</p>'
                    +'</div.>'
                    +'</div>'
                    +'<div style="width: 15%;height: 100%;float: right;">'
...
function liclick11(n) {
    ...
    localStorage.selectedRecord = JSON.stringify(reports[n]);
    console.log(reports[n])
    window.location.href="MyRecordDetail.html";
...
function liclick22(n) {


    localStorage.selectedRecord = JSON.stringify(workphotos[n]);
    console.log(workphotos[n])
    window.location.href="MyDetailworkphoto.html";
...
function liclick33(n) {


    localStorage.selectedRecord = JSON.stringify(ziliao[n]);
    console.log(ziliao[n])
    window.location.href="MyDetailziliao.html";
...
function pullUpAction() {
    ...
    //                     }
    //                     $("#thelist").append('<li onclick="liclick11(\''+i+'\')" style="list-style: none;margin: 0 0 20px 10px;border-bottom:1px solid #616161;padding-left: 0px;" >'
    //                         +'<p style="color: #333333;font-size: 18px;margin-left: 0px">'+"编号："+arr[i]['ID']+'</p>'
