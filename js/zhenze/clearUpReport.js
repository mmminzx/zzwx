...
var ZYFILE = {
    ...
    funUploadFiles: function () {
        ...
        if (selectedtype==0)
        {
            $("#myPopup1").html(" <h2 style=\"text-align:center;\">提示</h2><p>请选择上传问题类型</p>")
            $("#myPopup1").popup('open');
        ...
        else
        {
            ...
            console.log("Ashx_Question.ashx?method=reportQuestion&token="+token+"&riverCode="+getParam("OID")+"&riverName="+getParam("riverName")+"&reportUserName="+userName+"&qType=2&lon="+lon+"&lat="+lat+"&address="+riverName+"&rType="+selectedtype+"&emergency="+emergency+"&processContent="+ $("#question").val()+"&imgs="+images+"&videos=test&industryImg1=test&industryImg2=test&industryImg3=test&industryImg4=test&floater=test&ilegal=test&speed=test&guardrail=test&sewage=test&riverStatus=test&maintain=test");
            $.ajax({
                type: "get",
                url: svc_own + "Ashx_Question.ashx?method=reportQuestion&token="+token+"&riverCode="+riverId+"&riverName="+riverName+"&reportUserName="+userName+"&qType=2&lon="+lon+"&lat="+lat+"&address="+riverName+"&rType="+selectedtype+"&rTypeName="+selectedtypeName+"&emergency="+emergency+"&processType="+processType+"&content="+$("#question").val()+"&processContent="+ $("#question").val()+"&imgs="+images+"&videos=test&industryImg1=test&industryImg2=test&industryImg3=test&industryImg4=test&floater=test&ilegal=test&speed=test&guardrail=test&sewage=test&riverStatus=test&maintain=test",
                dataType: 'jsonp',
                jsonp: 'callback',
                data: null,
